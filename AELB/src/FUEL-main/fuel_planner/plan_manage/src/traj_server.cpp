...
/* std_msgs::Empty 类型的消息作为触发重新规划的信号。std_msgs::Empty 是一个简单的 ROS 消息类型，通常用于表示没有有效负载的空消息。*/
/*这段代码的作用是在收到重新规划信号后，计算并更新当前轨迹的结束时间，以便在合适的时间点结束当前轨迹。*/
void replanCallback(std_msgs::Empty msg) {
  // Informed of new replan, end the current traj after some time
  //表示在收到重新规划信号后，等待的最长时间（秒），之后会结束当前轨迹。
...
/*确保 traj_real_ 的长度不超过一定的限制（这里是 10000）。
如果长度超过限制，就删除最旧的一部分数据，以保持长度在合理范围内*/
void odomCallbck(const nav_msgs::Odometry& msg) {
  //检查 msg 中的 child_frame_id 是否为 "X" 或 "O"，说明这是某些特殊信息而不是实际里程计数据，因此直接返回，不进行处理。
...
/*何姿态消息回调函数，将接收到的姿态消息转换为旋转矩阵和平移向量的形式*/
...
void bsplineCallback(const bspline::BsplineConstPtr& msg) {
  ...
  //检查接收到的轨迹是否具有与先前接收到的轨迹相比的升序traj_id。
  //如果不是，则打印错误消息并返回，表示轨迹顺序错误。
  ...
  //这一部分使用从接收到的消息中解析的结点值、位置点等参数创建了B样条对象
  ...
  //清除任何现有的轨迹，并将新创建的轨迹存储到一个容器中（traj_）。
  //这些轨迹包括位置、速度和加速度，分别对应位置和偏航。
  traj_.clear();
...
void cmdCallback(const ros::TimerEvent& e) {
  ...
  //检查是否已接收到轨迹数据。如果没有接收到轨迹数据，那么不执行后续的轨迹控制和指令发布。
  if (!receive_traj_) return;
  //计算当前时刻相对于轨迹起始时间的时间差 t_cur
  ros::Time time_now = ros::Time::now();
  double t_cur = (time_now - start_time_).toSec();
  Eigen::Vector3d pos, vel, acc, jer;
  double yaw, yawdot;
  //如果当前时间在规划轨迹的范围内，则从轨迹中获取相应的状态信息；
  //如果当前时间超过轨迹范围，则保持发布最终位置和偏航角，并输出整个飞行过程的信息。
  ...
  //行了循环校正（isLoopCorrection 为真），则对位置、速度、和加速度进行循环校正。
  //循环校正的目的是调整位姿信息，使得飞行器在整个轨迹上具有良好的连续性。
  if (isLoopCorrection) {
