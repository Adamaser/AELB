...
//里程计信息接收回调函数，每收到里程计，就发布一个路径点可视化
void odom_callback(const nav_msgs::Odometry::ConstPtr& msg) {
  ...
  if (waypointSegments.size()) {
    ...
    //当前接收到的里程计消息的时间戳是否大于等于路径段队列中的第一个路径段的时间
...
//目标点获取回调函数，根据接收到的目标点信息，生成相应的路径点并发布。
void  goal_callback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
  ...
  /*
如果是 "circle"，执行 waypoints = circle();，生成一个圆形的路径点。
如果是 "eight"，执行 waypoints = eight();，生成一个八字形的路径点。
如果是 "point"，执行 waypoints = point();，生成一个点的路径点。
如果是 "series"，执行 load_waypoints(n, trigged_time);，从参数中加载预定义的路径段。
如果是 "manual-lonely-waypoint"，根据目标点的高度判断是否为有效目标点，
则将目标点添加到 waypoints 中，并发布路径点。
  */
  if (waypoint_type == string("circle")) {
    ...
    /*
    对于其他情况，判断目标点的高度：
    如果高度大于 0，表示是一个正常的目标点，将目标点添加到 waypoints 中，并发布路径点。
    如果高度在 0 和 -1.0 之间，表示需要移除最后一个目标点。
    如果高度小于 -1.0，表示目标点输入结束，发布路径点。
    */
...
void traj_start_trigger_callback(const geometry_msgs::PoseStamped& msg) {
  ...
  n.param("waypoint_type", waypoint_type, string("manual"));
/*
如果是 "free"，执行 waypoints = point();，生成一个自定义的路径点（可能是一个点）。
如果是 "circle"，执行 waypoints = circle();，生成一个圆形的路径点。
如果是 "eight"，执行 waypoints = eight();，生成一个八字形的路径点。
如果是 "point"，执行 waypoints = point();，生成一个点的路径点。
如果是 "series"，执行 load_waypoints(n, trigged_time);，从参数中加载预定义的路径段。
